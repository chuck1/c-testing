#include <Model/Model_3.h>

void Model_3::PopulateVariableMap()
{
...
void Model_3::SetVolumeConditions()
{
	PRNTLOC;
	...
	*/
	...
	PRNTLOC;
	// channel layer
	SetVolumeCondition( m_VC_fluid, 	IS(IS::e_ODD),				IS(IS::e_RANGE,0,4),	IS(IS::e_VALUE,1) );
	
	// first header layer
	SetVolumeCondition( m_VC_fluid, 	IS(IS::e_TRIM,	1,1),		IS(IS::e_VALUE,0),		IS(IS::e_VALUE,2) ); 	// outlet slot
	SetVolumeCondition( m_VC_fluid, 	IS(IS::e_TRIM,	1,1),		IS(IS::e_VALUE,4),		IS(IS::e_VALUE,2) );	// inlet slot
	
	// second header layer
	SetVolumeCondition( m_VC_fluid, 	IS(IS::e_TRIM,	1,0),		IS(IS::e_RANGE,0,1),	IS(IS::e_VALUE,3) ); 	// outlet slot	
	SetVolumeCondition( m_VC_fluid, 	IS(IS::e_TRIM,	0,1),		IS(IS::e_RANGE,3,5),	IS(IS::e_VALUE,3) );	// inlet slot
	PRNTLOC;
...
void Model_3::SetBoundaryConditions()
{
	PRNTLOC;
	// initialize
	...
	*/
	// inlet
	SetBoundaryCondition( m_BC_inlet,1,			IS(IS::e_VALUE,0),	IS(IS::e_VALUE,0),			IS(IS::e_RANGE,3,5),	IS(IS::e_VALUE,3) );
	
	// outlet
	SetBoundaryCondition( m_BC_outlet,-1,		IS(IS::e_VALUE,0),	IS(IS::e_END),				IS(IS::e_RANGE,0,1),	IS(IS::e_VALUE,3) );
	
	// symmetry
	SetBoundaryCondition( m_BC_sym,1,			IS(IS::e_VALUE,1),	IS(IS::e_ALL),				IS(IS::e_BEGIN),		IS(IS::e_ALL) );
	
	// heated
	SetBoundaryCondition( m_BC_heated,1,		IS(IS::e_VALUE,2),	IS(IS::e_TRIM,1,1),			IS(IS::e_RANGE,0,4),	IS(IS::e_BEGIN) );
