...
void JSL::Master::scan_event_devices()
{	
	int fd;
	char filename[64];
	...
	for(int i = 0; i < 32; i++)
	{
		...
		if(it != event_device_list_.end())
		{
			...
			std::shared_ptr<JSL::Event> device(new JSL::Event);
			device->fd_ = fd;
...
std::shared_ptr<JSL::Event> JSL::Master::find_event_device(JSL::event_device_id id)
{
	auto it = event_devices_.find(id);

	if(it != event_devices_.end()) return (*it).second;

	return std::shared_ptr<JSL::Event>();
}
int JSL::Master::device_open(char * filename)
{
	int fd, version;
...
JSL::event_device_id JSL::Master::device_info(int fd)
{
	struct input_id id;
