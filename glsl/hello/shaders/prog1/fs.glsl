...
void main(void)
{
	...
	for (int i = 0; i < light_count; i++) // for all light sources
	{
		...
		if((shadow_coor.w < 0.0) || (dist_from_light > shadow_coor_w_divide.z)) // not in shadow?
		{
			shadow = 0.0;
		}
		else // in shadow
		...
		if(shadow == 0.0) // not in shadow
		{
			...
			diffuse = atten * lights[i].diffuse * front.diffuse * color_tex * vec4(max(0.0, dot(fs_in.N,L)),1.0);
			...
			else // light source in front
			{
				specular = atten * lights[i].specular * front.specular *
					vec4(pow(max(0.0, dot(reflect(-L,N), -fs_in.P)), front.shininess),1.0);
			}
		}
		
		color = color + ambient + diffuse + specular + emission;
	}
}
