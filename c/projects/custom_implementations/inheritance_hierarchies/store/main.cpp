 ...
 * pattern to...
 *
 * in a class hierarchy, store at each level a pointer to an object in a parallel hierarchy (like base actors and physx actor for example)
 * and with a single call to `set` at the bottom of the heirarchy (most derived), set the pointers at each level (via the Link class)
 */
