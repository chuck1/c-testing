...
typedef map<string,shared_ptr<IVariable<double> > >::iterator mapVarDit;
typedef map<string,shared_ptr<IVariable<string> > >::iterator mapVarSit;
...
public:
	...
	void Init()
	{
		mapVarD["u"] = shared_ptr<u>(new u(this));
		mapVarD["up"] = shared_ptr<up>(new up(this));
		mapVarD["umean"] = shared_ptr<umean>(new umean(this));
	}
	
	shared_ptr<IVariable<double> > GetVarD(string str)
	{
		mapVarDit it = mapVarD.find(str);
		
		if ( it == mapVarD.end() )
		{
			return shared_ptr<IVariable<double> >();
		}
		
		shared_ptr<IVariable<double> > p = it->second;
	...
	shared_ptr<IVariable<string> > GetVarS(string str)
	{
		mapVarSit it = mapVarS.find(str);
		
		if ( it == mapVarS.end() )
		{
			return shared_ptr<IVariable<string> >();
		}
		
		shared_ptr<IVariable<string> > p = it->second;
...
private:
	...
	*/
	...
	// Objects
	map<string,shared_ptr<IVariable<double> > >		mapVarD;
	map<string,shared_ptr<IVariable<string> > >		mapVarS;
	
	/*
	shared_ptr<u>			m_u;
	shared_ptr<umean>		m_umean;
	shared_ptr<up>			m_up;
	shared_ptr<filenames>	m_filenames;
	*/
};
/*
