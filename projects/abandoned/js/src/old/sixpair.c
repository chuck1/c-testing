...
void set_master(usb_dev_handle *devh, int itfnum, int mac[6])
{
	...
	int res = usb_control_msg( devh, USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE, 0x09, 0x03f5, itfnum, msg, sizeof(msg), 5000);
...
void process_device(int argc, char **argv, struct usb_device *dev, struct usb_config_descriptor *cfg, int itfnum)
{
	...
	if ( argc >= 2 )
	{
		if ( sscanf(argv[1], "%x:%x:%x:%x:%x:%x", &mac_addr[0], &mac_addr[1], &mac_addr[2], &mac_addr[3], &mac_addr[4], &mac_addr[5]) != 6 )
		{
	...
	set_master(devh, itfnum, mac_addr);
			...
			for ( cfg = dev->config; cfg < dev->config + dev->descriptor.bNumConfigurations; ++cfg )
			{
				...
				{					
					...
					for ( alt = itf->altsetting; alt < itf->altsetting + itf->num_altsetting; ++alt )
					{
						...
						if ( dev->descriptor.idVendor == VENDOR && dev->descriptor.idProduct == PRODUCT && alt->bInterfaceClass == 3 )
						{
